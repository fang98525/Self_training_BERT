...
if __name__ == '__main__':
    ...
    corpus_path = '/data1/home/fzq/projects/self_pretrained/dataset/'
    # 加载数据
    unlabeled_df = pd.read_csv(data_path + 'nCoV_900k_train_unlabled.csv', encoding='utf_8_sig')
    # 清洗数据
    additional_chars = set()   #数据集中出现过的所有非中英文的数字符号集合
    ...
    #difference()方法用于返回集合的差集   属于集合一但是不属于集合二的元素
    additional_chars = additional_chars.difference(extra_chars)
    #调用stop——words方法   进行数据清理
    ...
    print("最终的前100条数据为",corpus_list[:100])
    #保存数据到目标位置
    ...
    print(len(count_1024) / row_len)  #分布依次为 0。39  0.29 0.31  0.00017  0.00001 .。。
